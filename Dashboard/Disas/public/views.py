...
def home(request):
    ...
    context = {
        'title': 'FutureGuard - Protect Your Community',
        'counties': counties,
        'current_year': 2025,
    }
...
def api_predict(request):
    if request.method == 'POST':
        try:
            ...
            if not county:
                return JsonResponse({'error': 'County required'}, status=400)
            
            predictions = prediction_service.predict_all(county)
    ...
    if request.method == 'POST':
        try:
            ...
            for disease, info in predictions.items():
                if disease not in ['timestamp', 'county']:
